...
import Jsp_Project.Movie_Ticket.Repository.MovieRepository;
import Jsp_Project.Movie_Ticket.Repository.ScreenRepository;
import Jsp_Project.Movie_Ticket.Repository.SeatRepository;
import Jsp_Project.Movie_Ticket.Repository.ShowRepository;
...
import Jsp_Project.Movie_Ticket.dto.SeatRowDto;
...
import Jsp_Project.Movie_Ticket.entity.ShowSeat;
...
import Jsp_Project.Movie_Ticket.util.CloudinaryHelper;
...
public class UserServiceImpl implements UserService {
	...
	public String editmovie(Long id, HttpSession session, ModelMap map, RedirectAttributes attributes) {
		...
		if (user == null || !user.getRole().equals("ADMIN")) {
			...
			MovieDto movieDto = new MovieDto(movie.getName(), movie.getLanguages(), movie.getGenre(),
					movie.getDuration(), null, movie.getTrailerLink(), movie.getDescription(), movie.getReleaseDate(),
					movie.getCast());
	...
	public String updateMovies(@Valid MovieDto movieDto, Long id, BindingResult result, RedirectAttributes attributes,
		...
		if (user == null || !user.getRole().equals("ADMIN")) {
			...
			movieRepository.save(movie);
			attributes.addFlashAttribute("pass", "Update data");
			return "redirect:/edit-movies/" + id;
		}
	}
	...
	public String loadMain(ModelMap map) {
		List<Show> shows = showRepository.findByShowDateAfter(LocalDate.now().minusDays(1));
	...
	@Override
	public String bookTickets(Long id, ModelMap map, RedirectAttributes attributes, HttpSession session) {
		User user = getUserFromSession(session);
		if (user == null || !user.getRole().equals("USER")) {
			attributes.addFlashAttribute("fail", "First Login Go And Login");
			return "redirect:/login";
		} else {
			Movie movie = movieRepository.findById(id).orElseThrow();
			List<Show> shows = showRepository.findByMovie(movie);
			map.put("movie", movie);
			map.put("Showdate", shows.stream().map(x -> x.getShowDate()).collect(Collectors.toList()));
			return "UserBook-ticket.html";
